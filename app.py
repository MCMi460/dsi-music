...
ask = lambda query: [
    i.split(")")[0].lower() for i in query.split("(") if ")" in i
].index(input(query).lower())
...
if __name__ == "__main__":
    match ask("(S)potify, (A)pple Music, (Y)ouTube Music, or (M)anual?\n> "):
        ...
        case 1:
            ...
            input(
                "Start playing a song in your desired"
        ...
        case 2:
            music.ytm.download(
                ...
                indirect=input(
                    "Would you like to indirectly match the track(s)? ... Results vary.\n(Y/N) > "
                ).lower()
                == "y",
            )
            quit()
